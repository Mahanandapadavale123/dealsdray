...
const CreateEmployee= async (req, res) => {
    ...
    if (req.files && req.files.image) {
        image = `/public/uploads/${req.files.image[0].filename}`;  // Store the relative URL for the image
    }
    ...
    if (courses && Array.isArray(courses)) {
        if (courses.length === 0) {
            return res.status(400).json({"status":false, message: "Atleast one course required" });
        }
    }else{
...
const allEmployee = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ status:true, message: "Employee Fetched Successfully",  data: employees, });
...
const updateEmployee = async (req, res) => {
    ...
    try {
        ...
        if (req.files && req.files.image) {
            ...
            updateData = { name, email, mobileNo, designation, gender, course, image };
        }else{
            updateData = { name, email, mobileNo, designation, gender, course };
        }
...
const changeEmployeeStatus = async (req, res) => {
    ...
    try {
        ...
        await employee.save();
        // Respond with the updated employee and new status
