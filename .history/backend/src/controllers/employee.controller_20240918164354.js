...
import { Employee } from "../models/employee.model.js"
 ...
 const CreateEmployee= async (req, res) => {
    ...
    console.log(f_Id, f_Image, f_Name, f_Email,f_Mobile, f_Designation, f_gender,f_Course,f_Createdate  );
    ...
    try {
        ...
        const employee = new Employee({ f_Id, f_Image, f_Name, f_Email,f_Mobile, f_Designation, f_gender,f_Course,f_Createdate });
        const savedEmp = await employee.save();

        return res.status(200).json({ emp: savedEmp, message: "Employee register saved In Successfully" });
...
const editEmployee=async (req, res) => {
    ...
    // try {
    //     await Employee.findByIdAndUpdate(req.params.id, { name, email, mobileNo, designation,Gender,course,image});
...
export  {CreateEmployee, editEmployee, allEmployee, deleteEmployee}
