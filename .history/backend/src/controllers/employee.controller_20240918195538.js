...
const CreateEmployee = async (req, res) => {
  const { name, email, mobileNo, designation, gender, course, image } =
    req.body; //f_Id, f_Image, f_Name, f_Email,f_Mobile, f_Designation, f_gender,f_Course,f_Createdate
  ...
  if (!mobileNoPattern.test(mobileNo)) {
    return res
      .status(400)
      .json({ message: "A valid mobile number is required (10 digits)" });
  }

  if (typeof designation !== "string") {
    return res
      .status(400)
      .json({ message: "Designation must be a valid string" });
  }
  ...
  if (!["MCA", "BSC", "BCA"].includes(course)) {
    return res
      .status(400)
      .json({ message: "Course is must be a valid Courses" });
  }

  try {
    const isExist = Employee.findOne({ mobileNo: mobileNo });
    if (isExist) {
    ...
    return res
      ...
      .json({
        emp: savedEmp,
        message: "Employee register saved In Successfully"
      });
  } catch (err) {
    console.log(err);
...
const deleteEmployee = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Employee and associated image deleted successfully" });
  } catch (err) {
