...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    // Check if the file type is either png or jpg/jpeg
    if (file.mimetype === 'image/png' || file.mimetype === 'image/jpeg') {
      cb(null, true); // Accept the file
    } else {
      cb(new Error('Only .png and .jpg image files are allowed!'), false); // Reject the file
    }
  }
});
...
router.route('/:id').put(upload.fields([{ name: 'avatar', maxCount: 1 }]) ,updateEmployee);

router.route ('/:id').delete(deleteEmployee);

router.route ('/status/:id').post(deleteEmployee);
